...
diffSummary <- function(a,b){
  ...
  jitter <- if (lastrow > .5) function(x){x - (rnorm(length(x),4,3))} else function(x){x + (rnorm(length(x),4,3))}
  z <- jitter(z)
  	zbar <- apply(z,2,
                  function(x){
                      c(mean(x,na.rm=TRUE),
...
combineHouse <- function(tmp){
    ...
    for(i in 1:nrow(out)) {
      out[i,"est"] <- round(tmp[[out[i,"who"]]][["delta"]][out[i,"pollster"], "Estimate"], 4)*100
      out[i,"lo"] <- round(tmp[[out[i,"who"]]][["delta"]][out[i,"pollster"], "2.5%"], 4)*100
      out[i,"hi"] <- round(tmp[[out[i,"who"]]][["delta"]][out[i,"pollster"], "97.5%"], 4)*100
      out[i,"dev"] <- round(tmp[[out[i,"who"]]][["delta"]][out[i,"pollster"], "StdDev"], 4)*100
    }
...
combineHouse2 <- function(house){
	housecontrast$est <- housecontrast$est - (goodmean*100)
...
write.csv(combineHouse(tmp),
          file=paste(dataDir,"/prelimhouse.csv",sep=""))

## combine response options FOR TESTING ONLY
#out <- combine(tmp)

#write.csv(out,
 #         file=paste(dataDir,"/prelimout.csv",sep="")) #FOR TESTING ONLY

#out <- read.csv(paste('data/',chart,'/prelimout.csv',sep=''))


###############Get house effect for "good" houses, designate R and D candidates plus order in contrast

house <- read.csv(paste(dataDir,'/prelimhouse.csv',sep=''))
...
goodmean <- (sum(goodmeanC)/sum(Goodfreqs))/100 #Divide by 100 since HEs in percents and xi in prob
goodmean[length(theGoodPollslist) < 2] <- goodmean/2
hlfmean <- goodmean/2


######Replace state-specific candidate name with generic R or D

testR <- (names(tmp)==RCand)
testD <- (names(tmp)==DCand)
names(tmp) <- replace(names(tmp), testR=="TRUE", "Republican")
names(tmp) <- replace(names(tmp), testD=="TRUE", "Democrat")


#with R cand first, negative is a shift toward the Dem, positive is a shift toward R
	#if negative, we want to ADD to the Dem and subtract from the R
	#if positive, we want to SUBTRACT from the Dem and add to the R

#with D cand first, negative is a shift toward the Rep, positive is a shift toward D
	#if negative, we want to ADD to the Rep and subtract from the D
	#if positive, we want to SUBTRACT from the Rep and add to the D
		#if coefficient is R, we subtract negative and add positive
		#if coefficient is D, we add negative and subtract positive
nRxis <- length(tmp$Republican$xi)
nDxis <- length(tmp$Democrat$xi)

for (i in 1:nRxis) {
tmp$Republican$xi[i] <- ifelse(candOrder=="Rfirst", tmp$Republican$xi[i] + hlfmean, tmp$Republican$xi[i] - hlfmean)
}
for (i in 1:nDxis) {
tmp$Democrat$xi[i] <- ifelse(candOrder=="Dfirst", tmp$Democrat$xi[i] + hlfmean, tmp$Democrat$xi[i] - hlfmean)
}
names(tmp) <- replace(names(tmp), names(tmp)=="Republican", RCand)
names(tmp) <- replace(names(tmp), names(tmp)=="Democrat", DCand)
...
if(n.Contrasts2>0){
  ...
  names(outContrasts2)[names(outContrasts2)=="date"] <- "date14"
...
houseOut <- combineHouse2(house)

write.csv(houseOut,
          file=paste(dataDir,"/nathouse.csv",sep=""))

houseOut$est <- houseOut$est/100
houseOut$lo <- houseOut$lo/100
houseOut$hi <- houseOut$hi/100
houseOut$dev <- houseOut$dev/100
